...
int d[110];
bool vis[110];
int wait(int num[],int len,int n)
{
	int a=n-num[0];
	int b=num[len-1]-n;
	if(a>b)
	...
	{	
		...
		for(j=0;j<el;j++)
		{
			...
			for(l=0;l<k-1;l++)
			{
				for(m=l+1;m<k;m++)
				{		
					vc[num[l]].push_back(state(num[m],5+elct[j]*(abs(num[l]-num[m])+wait(num,k,num[l]))));
					vc[num[m]].push_back(state(num[l],5+elct[j]*(abs(num[l]-num[m])+wait(num,k,num[m]))));
				}
			}
		}
		if(to == 0)
