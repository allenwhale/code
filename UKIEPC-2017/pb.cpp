...
int main(){
    ...
    //while(cmp(p[i].Dist(p[(b + 1) % N]) - p[i].Dist(p[b])) >= 0)
    //b = (b + 1) % N;
    //ans = min(ans, p[i].Dist(p[b]));
    //}
    for(int i = 0, c = 1 ; i < N ; i++){
        int a = i, b = (i + 1) % N;
        while(cmp(Line(p[a], p[b]).Dist(p[(c + 1) % N]) - Line(p[a], p[b]).Dist(p[c])) >= 0)
            c = (c + 1) % N;
        ans = min(ans, Line(p[a], p[b]).Dist(p[c]));
    }
    //for(int i = 0 ; i < N ; i++){
        //int a = i, b = (i + 1) % N;
        //double tmp = 0;
        //for(int j = 0 ; j < N ; j++)
            //tmp = max(tmp, Line(p[a], p[b]).Dist(p[j]));
        //ans = min(ans, tmp);
    //}
    printf("%.10f\n", ans);
    return 0;
