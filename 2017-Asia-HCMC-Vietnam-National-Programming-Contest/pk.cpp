...
#define ONEDGE  2
#define OUTSIDE 0
    int OnPolygon(const Point &n) const {
...
bool check_inside(const Polygon &con, const vector<Point> &ps){
    for(auto &p : ps){
        if(con.OnPolygon(p) == INSIDE)
            return true;
    }
    return false;
}
...
int main(){
    ...
    for(int i = 0 ; i < M ; i++){
        double x, y;
        scanf("%lf%lf", &x, &y);
        castle.push_back({x, y});
    }
    Polygon con_1(N);
    for(auto &p : soldier)
        con_1.add(p);
    con_1 = con_1.ConvexHull();
    unordered_set<Point, pair_hash> ans;
    if(con_1.N >= 4 || check_inside(con_1, soldier)){
        for(auto &p : castle){
            int res = con_1.OnPolygon(p);
        ...
        for(int i = 0 ; i < 3 ; i++){
            Polygon con_2;
            for(int j = 0 ; j < N ; j++){
                if(!(soldier[j] == con_1.s[i])){
                    con_2.add(soldier[j]);
                }
            }
            con_2 = con_2.ConvexHull();
            if(con_2.N >= 4){
                for(auto &p : castle){
                    int res = con_2.OnPolygon(p);
                    if(res == INSIDE || res == ONEDGE){
                        ans.insert(p);
                    }
                }
